...
enum LocalLibrary {
    ...
    static func importPackage(_ source:URL)throws->MatchItem {
        ...
        let required=[manifest.media.path,manifest.artifacts["frames"]!] + (isV2 ? [manifest.artifacts["events"]!,manifest.artifacts["corrections"]!] : [])
        ...
        do {
            ...
            if FileManager.default.fileExists(atPath:eventsURL.path) {
                let events=try ContractJSON.read(EventCollection.self,from:eventsURL)
    ...
    static func remove(_ item:MatchItem)throws { try FileManager.default.removeItem(at:item.folder) }
